          ...
          return (
            <div key={p} className="flex items-center gap-1 shrink-0">
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all duration-300 ${
                done ? 'bg-[#EEF1FB] text-[#1434CB]' :
                active ? 'bg-[#FCC015]/20 text-black' :
                ...
                {active && (
                  <motion.span
                    ...
                    className="inline-block w-3 h-3 border-2 border-[#1434CB] border-t-transparent rounded-full"
                  />
                )}
                {PHASE_LABELS[p].split('...')[0].split(' ').slice(0, 2).join(' ')}
              </div>
              {i < PHASE_ORDER.length - 1 && (
                <div className={`w-4 h-px ${done ? 'bg-[#1434CB]/40' : 'bg-slate-200'}`} />
              )}
            </div>
          );
        })}
        ...
        {phase !== 'idle' && (
          ...
          >
            ...
            <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
              <motion.div
                className={`h-full rounded-full ${isDone ? 'bg-[#1434CB]' : 'bg-[#1434CB]'}`}
